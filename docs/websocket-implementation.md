...
- **`QueueControlBar` — `TOUR_OPEN_QUEUE_DRAWER_EVENT` + `TOUR_CLOSE_PLAY_VIEW_EVENT` window events.** The bar listens for both so the tour can programmatically open/close its internal drawers (`onboarding:open-queue-drawer`, `onboarding:close-play-view`).
- **`QueueControlBar` session mini-bar — `data-tour-anchor="session-mini-bar"`.** The clickable row inside the queue control that opens `SeshSettingsDrawer`; the tour anchors its "Open your session" step to this attribute.
...
- **`PlayViewDrawer` — `data-tour-anchor="play-view-drawer"`.** Anchor for the "Everything for this climb" step.
- **`StartSeshDrawer` — `data-tour-anchor="start-sesh-drawer"`.** Anchor for the board-pick step.
