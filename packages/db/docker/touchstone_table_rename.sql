-- Rename tables with touchstone_ prefix
DO $$
DECLARE
    r RECORD;
BEGIN
    ...
    LOOP
        EXECUTE 'DROP TABLE IF EXISTS ' || quote_ident('touchstone_' || r.tablename) || ' CASCADE';
        EXECUTE 'ALTER TABLE ' || quote_ident(r.tablename) ||
                ' RENAME TO ' || quote_ident('touchstone_' || r.tablename);
    END LOOP;
END $$;

-- Migrate Touchstone data to unified tables
-- Level 0: Tables with no foreign key dependencies
-- -----------------------------------------------------------------------------

INSERT INTO board_attempts (board_type, id, position, name)
SELECT 'touchstone', id, position, name FROM touchstone_attempts
...
SELECT 'touchstone', difficulty, boulder_name, route_name, is_listed FROM touchstone_difficulty_grades
...
SELECT 'touchstone', id, name, is_listed, password, min_count_in_frame, max_count_in_frame FROM touchstone_products
ON CONFLICT (board_type, id) DO NOTHING;

INSERT INTO board_sets (board_type, id, name, hsm)
SELECT 'touchstone', id, name, hsm FROM touchstone_sets
ON CONFLICT (board_type, id) DO NOTHING;

INSERT INTO board_users (board_type, id, username, created_at)
SELECT 'touchstone', id, username, created_at FROM touchstone_users
ON CONFLICT (board_type, id) DO NOTHING;
...
SELECT 'touchstone', id, product_id, name, instagram_caption, is_mirrored, is_listed, password, created_at FROM touchstone_layouts
ON CONFLICT (board_type, id) DO NOTHING;
...
SELECT 'touchstone', id, product_id, edge_left, edge_right, edge_bottom, edge_top, name, description, image_filename, position, is_listed FROM touchstone_product_sizes
ON CONFLICT (board_type, id) DO NOTHING;

INSERT INTO board_holes (board_type, id, product_id, name, x, y, mirrored_hole_id, mirror_group)
SELECT 'touchstone', id, product_id, name, x, y, mirrored_hole_id, mirror_group FROM touchstone_holes
ON CONFLICT (board_type, id) DO NOTHING;

INSERT INTO board_placement_roles (board_type, id, product_id, position, name, full_name, led_color, screen_color)
SELECT 'touchstone', id, product_id, position, name, full_name, led_color, screen_color FROM touchstone_placement_roles
ON CONFLICT (board_type, id) DO NOTHING;
...
SELECT 'touchstone', id, product_size_id, hole_id, position FROM touchstone_leds
ON CONFLICT (board_type, id) DO NOTHING;

INSERT INTO board_placements (board_type, id, layout_id, hole_id, set_id, default_placement_role_id)
SELECT 'touchstone', id, layout_id, hole_id, set_id, default_placement_role_id FROM touchstone_placements
ON CONFLICT (board_type, id) DO NOTHING;

INSERT INTO board_product_sizes_layouts_sets (board_type, id, product_size_id, layout_id, set_id, image_filename, is_listed)
SELECT 'touchstone', id, product_size_id, layout_id, set_id, image_filename, is_listed FROM touchstone_product_sizes_layouts_sets
ON CONFLICT (board_type, id) DO NOTHING;
...
SELECT uuid, 'touchstone', layout_id, setter_id, setter_username, name, description, hsm, edge_left, edge_right, edge_bottom, edge_top, angle, frames_count, frames_pace, frames, is_draft, is_listed, created_at, true, NULL
FROM touchstone_climbs
...
SELECT 'touchstone', uuid, user_id, name, product_id, is_adjustable, angle, layout_id, product_size_id, hsm, serial_number, created_at FROM touchstone_walls
...
SELECT 'touchstone', climb_uuid, angle, display_difficulty, benchmark_difficulty, ascensionist_count, difficulty_average, quality_average, fa_username, fa_at
FROM touchstone_climb_stats
...
SELECT 'touchstone', climb_uuid, link, foreign_username, angle, thumbnail, is_listed, created_at
FROM touchstone_beta_links
...
SELECT 'touchstone', table_name, last_synchronized_at FROM touchstone_shared_syncs
...
BEGIN
    FOR r IN
        ...
        FROM pg_tables
        WHERE schemaname = 'public'
        AND tablename LIKE 'touchstone_%'
    LOOP
